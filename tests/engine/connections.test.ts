...
describe('connection parity with engine v0.1.8', () => {
  it('requires a closed connection to be opened before movement', () => {
    ...
    const connection = state.connections.bedroom_hallway;
    if (!connection) throw new Error('missing test connection');
    connection.open = false;

    const moveWhileClosed = performAction(state, { id: 'MOVE', targetId: 'hallway' });
    ...
    expect(options.some((option) => option.id === 'MOVE' && option.targetId === 'hallway')).toBe(false);
    ...
    expect(opened.state.connections.bedroom_hallway?.open).toBe(true);
    expect(validateState(opened.state)).toEqual([]);

    const moved = performAction(opened.state, { id: 'MOVE', targetId: 'hallway' });
    expect(moved.result.success).toBe(true);
    expect(moved.result.elapsedSeconds).toBe(connection.travelSeconds);
    expect(moved.state.player.locationId).toBe('hallway');
  ...
  it('does not open or traverse a locked connection', () => {
    ...
    expect(getContextActions(state).some((option) => option.targetId === 'hallway' || option.targetId === connection.id)).toBe(false);
    expect(performAction(state, { id: 'OPEN_CONNECTION', targetId: connection.id }).result.success).toBe(false);
    expect(performAction(state, { id: 'MOVE', targetId: 'hallway' }).result.success).toBe(false);
  });

  it('rejects opening a non-adjacent connection', () => {
    const state = createInitialState();
    const connection = state.connections.kitchen_garage;
    if (!connection) throw new Error('missing test connection');
    connection.open = false;
    expect(performAction(state, { id: 'OPEN_CONNECTION', targetId: connection.id }).result.success).toBe(false);
  });
