...
interface HistoricalEngine { createInitialState(): Record<string, unknown> }
...
it('diagnoses historical v0.1.11 item shape', () => {
  ...
  const engine = moduleRef.exports as HistoricalEngine;
  const state = engine.createInitialState();
  throw new Error(JSON.stringify({ items: state.items, inventory: state.inventory, player: state.player }, null, 2));
});
