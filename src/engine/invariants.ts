...
export function validateState(state: GameState): InvariantViolation[] {
  ...
  if (!state.locations[state.player.locationId]) {
    errors.push(violation('PLAYER_LOCATION_MISSING', `Player location ${state.player.locationId} does not exist.`));
  }

  if (!isPercent(state.player.healthPv)) {
  ...
  for (const itemId of state.player.inventoryIds) {
    ...
    if (!item) {
      errors.push(violation('INVENTORY_ITEM_MISSING', `Inventory references missing item ${itemId}.`));
    } else if (item.location.kind !== 'inventory') {
      errors.push(violation('INVENTORY_LOCATION_MISMATCH', `${itemId} is listed in inventory but its location is ${item.location.kind}.`));
    }
  }
  ...
  for (const container of Object.values(state.containers)) {
    ...
    for (const itemId of container.contentIds) {
      ...
      const item = state.items[itemId];
      if (!item) {
        errors.push(violation('CONTAINER_ITEM_MISSING', `${container.id} references missing item ${itemId}.`));
      } else if (item.location.kind !== 'container' || item.location.id !== container.id) {
        errors.push(violation('CONTAINER_LOCATION_MISMATCH', `${itemId} is listed in ${container.id} but its item location disagrees.`));
      }
    }
  }
