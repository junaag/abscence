...
import { describeItemExamination } from '../examination';
...
export function fillLiquidContainer(state: GameState, itemId: string | undefined): EngineTransition {
  ...
  const current = item.liquidMl ?? 0; const capacity = item.capacityMl ?? 0;
  if (capacity <= current) return failure(state, 'Déjà pleine', 'La bouteille est déjà pleine.');
  const added = capacity - current; const elapsed = Math.max(8, Math.round(added / WATER_RULES.bottleFillMlPerSecond));
  const next = cloneState(state); const nextItem = next.items[itemId]; if (!nextItem) return failure(state, 'Impossible', 'Le contenant a disparu.');
  nextItem.liquidMl = capacity; advanceTime(next, elapsed);
...
export function useItem(state: GameState, itemId: string | undefined): EngineTransition {
  ...
  const item = state.items[itemId]; if (!item || item.location.kind !== 'inventory') return failure(state, 'Impossible', 'L’objet doit être transporté.');
  const definition = getItemDefinition(item.definitionId); if (!definition?.usable) return failure(state, 'Impossible', 'Cet objet n’a pas d’utilisation disponible.');
  const battery = definition.battery; const charge = item.batteryPercent ?? battery?.initialChargePct ?? 0;
  if (battery && charge <= 0) return failure(state, 'Batterie vide', 'L’appareil ne peut plus fonctionner.');
  const next = cloneState(state); const nextItem = next.items[itemId]; if (!nextItem) return failure(state, 'Impossible', 'Cet objet a disparu.');
...
export function chargeItem(state: GameState, itemId: string | undefined, sourceId: string | undefined, requestedSeconds: number | undefined): EngineTransition {
  ...
  const item = state.items[itemId]; const source = state.items[sourceId];
...
export function examineItem(state: GameState, itemId: string | undefined): EngineTransition {
  ...
  return success(next, `Vous examinez ${item.name.toLowerCase()}.`, describeItemExamination(next, itemId), 8);
}
