...
function parseTarget(value: string | undefined): MapTravelTarget | null {
  ...
  try {
    ...
    return {
      id: id.slice(0, 120), name: name.slice(0, 120), x, y,
...
function distance(state: GameState, target: MapTravelTarget): number {
  return getDistanceMeters(state, state.player.locationId, null, { x: target.x, y: target.y }) ?? Number.POSITIVE_INFINITY;
}
...
export function searchLocation(state: GameState): EngineTransition {
  ...
  const totalBaseSeconds = getPoiZoneSearchSeconds(zone), currentProgress = getPoiZoneSearchProgressSeconds(zone), remainingBaseSeconds = getPoiZoneSearchRemainingSeconds(zone);
  const baseChunkSeconds = Math.min(900, remainingBaseSeconds), seconds = scalePhysicalDuration(state, baseChunkSeconds, 'action');
...
export function walkToMapPoint(state: GameState, encodedTarget: string | undefined): EngineTransition {
  ...
  next.locations.map_walk_position = { id: 'map_walk_position', name: 'Rue / extérieur', ambientTemperatureC: origin?.ambientTemperatureC ?? 20, ambientHumidityPercent: origin?.ambientHumidityPercent ?? 50, ventilation: 1, features: {}, position: { x: target.x, y: target.y } };
  next.player.locationId = 'map_walk_position'; recordLocationVisit(next, 'map_walk_position');
  advanceTime(next, seconds); applyPhysicalExertion(next, seconds, 1);
...
export function travelToMapPoi(state: GameState, encodedTarget: string | undefined): EngineTransition {
  ...
  next.player.locationId = destinationId; recordLocationVisit(next, destinationId);
  advanceTime(next, seconds); applyPhysicalExertion(next, seconds, 1);
  return success(next, target.id === 'house_1' ? 'Retour au domicile' : target.name, `Vous parcourez environ ${Math.max(1, Math.round(meters))} m à pied.${loadNote(state)}`, seconds);
}
