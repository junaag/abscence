...
/**
 * Exact analytical helper inherited from engine v0.1.8.
 * It makes critical-need damage independent of simulation chunk size.
 */
...
/** Historical v0.1.8 PV damage budget. */
...
export function advancePhysiology(
  state: GameState,
  seconds: number,
  rates: Readonly<NeedsState> = BASE_RATES_PER_MINUTE,
): PhysiologyAdvanceResult {
...
export function advanceTime(state: GameState, seconds: number): PhysiologyAdvanceResult {
  const elapsedSeconds = Math.max(0, Number(seconds) || 0);
